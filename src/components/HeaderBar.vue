<script setup lang="ts">
import { ref, onMounted, onUnmounted } from 'vue';
import { useAuthStore } from '../stores/authStore';
import { useRouter } from 'vue-router';

const auth = useAuthStore();
const router = useRouter();
const isScrolled = ref(false);

// Scroll styling
const handleScroll = () => {
  isScrolled.value = window.scrollY > 50;
};

// Navigation
const navigateTo = (path: string) => {
  router.push(path)
  window.scrollTo({ top: 0, behavior: 'smooth' });
};

const handleCartClick = () => {
  if (!auth.user) {
    alert("Please sign in to view your shopping bag.");
    return;
  }
  navigateTo('/cart'); // allowed
};

// Mount scroll listener
onMounted(() => window.addEventListener('scroll', handleScroll));
onUnmounted(() => window.removeEventListener('scroll', handleScroll));
</script>

<template>
    <!-- HEADER --> 
     <header class="navbar" :class="{ scrolled: isScrolled }"> 
        <div class="top-bar"> 
            <div class="right-icons"> 
                <!-- SIGN-IN / USER INFO + SIGN OUT --> 
                <div> 
                    <!-- Loading --> 
                     <span v-if="auth.loading">Loading...</span> 
                     <!-- If NOT logged in --> 
                      <a v-else-if="!auth.user" href="#" class="signin" @click.prevent="auth.login" > Sign In </a> 
                      <!-- If logged in: show user --> 
                       <div v-else style="display: flex; align-items: center; gap: 0.5rem;"> 
                            <img :src="auth.user.photoURL" alt="User" style="width: 28px; height: 28px; border-radius: 50%;" /> 
                            <span>{{ auth.user.displayName }}</span> 
                            <button @click="auth.logout" style="background: none; border: none; color: #0032A0; cursor: pointer;" > Sign Out </button> 
                        </div> 
                    </div> 
                    <!-- CART BUTTON (protected) --> 
                     <a href="#" class="cart" @click.prevent="handleCartClick" > 
                        <img src="/images/cart-icon.png" alt="Shopping Cart" /> 
                    </a> 
                </div> 
            </div> 
            <div class="logo-container"> 
                <img src="/images/lakerstore-logo.png" alt="GVSU LakerStore Logo" class="logo-img" /> 
            </div> 
            <nav class="nav-menu"> 
                <ul> 
                    <li><router-link to="/">Home</router-link></li> 
                    <li><router-link to="/mens">Mens</router-link></li> 
                    <li><router-link to="/womens">Womens</router-link></li> 
                    <li><router-link to="/family">Family</router-link></li> 
                    <li> <router-link to="/accessories">Accessories</router-link> </li> 
                    <li><router-link to="/alumni">Alumni</router-link></li> 
                </ul> 
            </nav>
  </header>
</template>

<style scoped>
.navbar {
  /* Add your navbar styling here */
}

.scrolled {
  /* Add styles when scrolled */
}

.top-bar {
  display: flex;
  justify-content: flex-end;
  padding: 0.5rem 1rem;
}

.right-icons {
  display: flex;
  align-items: center;
  gap: 1rem;
}

.logo-container {
  display: flex;
  justify-content: center;
  margin: 1rem 0;
}

.nav-menu ul {
  display: flex;
  list-style: none;
  justify-content: center;
  gap: 1.5rem;
  padding: 0;
  margin: 0;
}
</style>
